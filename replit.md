...
- **Modular Risk Calculation Engine**: Framework-agnostic architecture with adapter pattern routing calculations to template-specific implementations. Supports:
  - **Executive Protection**: Risk_EP = (T × V × E) × I × (1 - C_e) with Exposure (E) factor weighted from public profile (40%), pattern predictability (30%), digital footprint (20%), and geographic risk (10%)
  - **Office Building**: Standard T×V×I formula with vulnerability calculated from interview risk indicators
  - **Retail Store**: T×V×I with likelihood adjusted by numeric shrinkage rates (industry baseline 1.4-1.6%)
  - **Warehouse**: T×V×I with cargo theft vulnerability and location-based likelihood adjustments
  - **Data Center/Manufacturing**: Fallback adapters (to be implemented)
  Engine uses compound reduction model (10% per effectiveness point) for control effectiveness calculations. Located in server/services/risk-engine/ with types.ts and framework adapters.
