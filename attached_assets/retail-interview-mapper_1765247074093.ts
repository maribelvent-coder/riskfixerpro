 ...
 * - TÃ—VÃ—I formula (standard facility assessment, scale 1-5 for each)
 ...
 * TÃ—VÃ—I scale with evidence chains
 */
export interface RetailAIAssessmentResponse {
  threatId: string;
  
  // TÃ—VÃ—I Scores (1-5 scale)
  threatLikelihood: {
    score: number;
    label: string;
    evidence: string[];
...
2. METHODOLOGY-COMPLIANT: Follow the ASIS GDL-RA framework using TÃ—VÃ—I where:
   ...
   - Inherent Risk = T Ã— V Ã— I (scale 1-125, normalized to 0-100)
...
Calculate: Inherent Risk = T Ã— V Ã— I (max 125, normalize to 0-100)
...
${topRisks.map(r => {
  ...
  return `- ${threat?.name || r.threatId}: T=${r.threatLikelihood.score}, V=${r.vulnerability.score}, I=${r.impact.score} â†’ Score: ${r.inherentRisk.normalizedScore.toFixed(1)}/100 (${r.inherentRisk.classification.toUpperCase()})`;
}).join('\n')}
