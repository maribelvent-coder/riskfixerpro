...
// Manufacturing interview imports
import {
  calculateVulnerabilityFromInterview as mfgCalculateVulnerability,
  calculateThreatLikelihoodFromInterview as mfgCalculateLikelihood,
  calculateImpactFromInterview as mfgCalculateImpact,
  calculateOverallSecurityScore as mfgCalculateSecurityScore,
  generateControlRecommendations as mfgGenerateControlRecs,
  calculateRiskForThreat as mfgCalculateRiskForThreat,
  initializeRiskScenariosFromInterview as mfgInitializeScenarios,
  MANUFACTURING_THREATS,
} from "./services/manufacturing-interview-mapper";

import {
  MANUFACTURING_INTERVIEW_QUESTIONS,
  getQuestionsBySection as mfgGetQuestionsBySection,
  getQuestionById as mfgGetQuestionById,
  getAllQuestionsFlattened as mfgGetAllQuestions,
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get manufacturing interview questions
  app.get(
    "/api/assessments/:id/manufacturing-interview/questions",
    verifyAssessmentOwnership,
    async (req, res) => {
      try {
        const questions = mfgGetAllQuestions();
        res.json({
          success: true,
          questions,
          totalQuestions: questions.length,
          sections: Object.keys(MANUFACTURING_INTERVIEW_QUESTIONS),
        });
      } catch (error) {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        res.json({
          success: true,
          threats: MANUFACTURING_THREATS,
          totalThreats: MANUFACTURING_THREATS.length,
        });
      } catch (error) {
        console.error("Error fetching manufacturing threats:", error);
        res.status(500).json({ error: "Failed to fetch threat list" });
      }
    },
  );

  // Calculate risk for a single manufacturing threat
  app.post(
    "/api/assessments/:id/manufacturing-interview/calculate-risk",
    verifyAssessmentOwnership,
    async (req, res) => {
      try {
        const { threatId, interviewResponses } = req.body;

        if (!threatId || !interviewResponses) {
          return res.status(400).json({
            error: "threatId and interviewResponses are required",
          });
        }

        const riskResult = mfgCalculateRiskForThreat(threatId, interviewResponses);
        res.json({
          success: true,
          ...riskResult,
        });
      } catch (error) {
        console.error("Error calculating manufacturing risk:", error);
        res.status(500).json({ error: "Failed to calculate risk" });
      }
    },
  );

  // Generate all risk scenarios from manufacturing interview
  app.post(
    "/api/assessments/:id/manufacturing-interview/generate-scenarios",
    verifyAssessmentOwnership,
    async (req, res) => {
      try {
        const assessmentId = req.params.id;
        ...
        if (!interviewResponses) {
          return res.status(400).json({
            error: "interviewResponses are required",
          });
        }

        // Initialize risk scenarios from interview responses
        // Note: assessmentId is a UUID string, pass as-is
        const scenarioResult = await mfgInitializeScenarios(assessmentId, interviewResponses);

        // Calculate overall security score
        const securityScore = mfgCalculateSecurityScore(interviewResponses);

        // Generate control recommendations
        const controlRecs = mfgGenerateControlRecs(interviewResponses);

        res.json({
          success: scenarioResult.success,
          assessmentId,
          scenarioCount: scenarioResult.scenarios?.length || 0,
          ...
          securityScore,
          controlRecommendations: controlRecs.slice(0, 10),
        });
      } catch (error) {
        console.error("Error generating manufacturing scenarios:", error);
        res.status(500).json({ error: "Failed to generate risk scenarios" });
      }
    },
  );

  // Get control recommendations for manufacturing assessment
  app.post(
    "/api/assessments/:id/manufacturing-interview/control-recommendations",
    verifyAssessmentOwnership,
    async (req, res) => {
      try {
        const { interviewResponses } = req.body;
        ...
        const recommendations = mfgGenerateControlRecs(interviewResponses);
        res.json({
          success: true,
          recommendations,
        ...
        console.error("Error generating manufacturing control recommendations:", error);
