...
export interface ManufacturingGeneratedScenarioResult {
  ...
  mediumRisks: number;
  lowRisks: number;
  riskScenarioIds: string[]; // Using string for pending IDs before persistence
...
export async function generateManufacturingRiskScenariosWithAI(
  ...
  // Note: Database persistence removed - route layer should handle via storage.ts
  // Results are returned for persistence by the calling route
  const riskScenarioIds: string[] = [];
  for (const result of results) {
    // Generate a temporary ID for tracking (actual persistence handled by route layer)
    riskScenarioIds.push(`pending-${result.threatId}`);
  }
...
// ============================================================================
// EXPORTS - Types already exported at declaration, functions are named exports
// ============================================================================
