...
export interface CargoTheftIncident {
  date: string;
  loss: number;
...
export interface CargoTheftROICalculatorProps {
  assessment: {
...
export function CargoTheftROICalculator({ 
  assessment, 
  proposedControls 
}: CargoTheftROICalculatorProps) {
  // ============================================================
  // CALCULATION LOGIC
  // ============================================================
  ...
  // 2. Average Historic Cargo Theft Loss
  const cargoTheftHistory = assessment.cargoTheftHistory || [];
  const historicTheftLoss = cargoTheftHistory.length > 0
    ? cargoTheftHistory.reduce((sum, incident) => sum + incident.loss, 0) / cargoTheftHistory.length
    : 0;
  
  // 3. Total Projected Annual Loss
  const projectedAnnualLoss = annualShrinkage + historicTheftLoss;
  
  // 4. Total Cost of Proposed Controls
  const totalControlCost = proposedControls.reduce(
    (sum, control) => sum + control.estimatedCost, 
    0
  );
  
  // 5. Total Reduction Percentage (capped at 85%)
  ...
  const projectedSavings = projectedAnnualLoss * totalReduction;
  
  // 7. Payback Period (in years)
  const paybackPeriod = totalControlCost > 0 ? totalControlCost / projectedSavings : 0;
  
  // 8. Three-Year ROI Percentage
  const threeYearROI = totalControlCost > 0
    ? (((projectedSavings * 3) - totalControlCost) / totalControlCost) * 100
    : 0;
  
  // 9. Determine if ROI is positive
  const isPositiveROI = (projectedSavings * 3) > totalControlCost;
  ...
  const shrinkageStatus = getShrinkageStatus(assessment.shrinkageRate);
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="destructive" className="gap-1">
              ...
              {formatPercentage(assessment.shrinkageRate)}
            </Badge>
          </div>
          <div className="text-4xl font-bold text-red-600">
            {formatCurrency(projectedAnnualLoss)}
          </div>
          <div className="text-xs text-muted-foreground">
            ${formatCurrency(annualShrinkage)} shrinkage + ${formatCurrency(historicTheftLoss)} avg. theft loss
          </div>
        </div>
        ...
        <div className="rounded-lg bg-muted/50 p-4 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">Proposed Security Investment</span>
            <span className="text-lg font-semibold">{formatCurrency(totalControlCost)}</span>
          </div>
          <div className="text-xs text-muted-foreground">
        ...
        <div className="space-y-2">
          ...
          <div className="text-xs text-muted-foreground">
            Reducing loss by {formatPercentage(totalReduction * 100)}
          </div>
        </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{formatPercentage(assessment.shrinkageRate)}</span>
                </div>
              </div>
              <Progress 
                value={Math.min(100, (assessment.shrinkageRate / 5) * 100)} 
                className="h-2"
              />
            </div>
        ...
        {proposedControls.length > 0 && (
          <div className="space-y-3 pt-4 border-t">
            <span className="text-sm font-semibold">Proposed Security Controls</span>
            <div className="space-y-2">
              {proposedControls.map((control, index) => (
                ...
                >
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Reduces risk by {formatPercentage(control.reductionPercentage)}
                    </div>
                  </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-1">
                ...
                <div className="text-xs text-blue-700/70 dark:text-blue-300/70">
                  Necessary investment for security risk mitigation & duty of care
                </div>
              </div>
